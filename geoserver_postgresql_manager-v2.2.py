"""
GeoServer & PostgreSQL 完整集成管理系统 - 修复版
修复线程管理问题，增强稳定性和用户体验
版本: V2.2
"""
...
from PyQt6.QtCore import (
    ...
    QObject, QMutex, QWaitCondition
)
from PyQt6.QtGui import QIcon, QFont, QPixmap, QPalette, QColor, QCloseEvent
...
class SafeWorker(QObject):
    ...
    def __init__(self, task_func, *args, **kwargs):
        ...
        self.args = args
        self.kwargs = kwargs
        self.logger = log_manager.get_logger('SafeWorker')
...
class ThreadManager(QObject):
    ...
    def __init__(self):
        ...
        self.thread_pool.setMaxThreadCount(4)  # 最大4个并发线程
        self.logger = log_manager.get_logger('ThreadManager')
    
    def start_task(self, task_func, *args, **kwargs):
        """启动任务"""
        worker = SafeWorker(task_func, *args, **kwargs)
        thread = QThread()
    ...
    def cancel_all(self):
        ...
        for thread, worker in self.active_threads:
            if worker:
                worker.cancel()
    ...
    def wait_for_all(self, timeout=10000):
        """等待所有线程完成"""
...
class ImprovedMainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GeoServer & PostgreSQL 数据管理系统 V2.2")
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        try:
            ...
            self.thread_manager.wait_for_all(5000)  # 等待5秒
    ...
    def setup_toolbar(self, layout):
        ...
        # 取消操作按钮
        self.cancel_btn = QPushButton(f"{get_icon('cancel')} 取消操作")
    ...
    def cancel_current_operation(self):
        ...
        try:
            ...
            self.logger.error(f"取消操作时发生错误: {e}")
    ...
    def enable_action_buttons(self, enabled):
        """启用/禁用操作按钮"""
        self.import_pg_btn.setEnabled(enabled)
        self.publish_gs_btn.setEnabled(enabled)
        self.import_publish_btn.setEnabled(enabled)
    ...
    def import_to_postgresql(self):
        ...
        try:
            ...
            def import_task():
                try:
                    ...
                    for i, item in enumerate(self.scanned_data):
                        ...
                        if self.current_worker:
                            self.current_worker.progress.emit(progress)
                            self.current_worker.status.emit(f"正在导入: {item['new_name']}")
                    ...
                    if self.current_worker:
                        self.current_worker.progress.emit(100)
            ...
            # 启动导入任务
            self.current_worker = self.thread_manager.start_task(import_task)
    ...
    def publish_to_geoserver(self):
        ...
        try:
            ...
            def publish_task():
                try:
                    ...
                    def progress_callback(value):
                        if self.current_worker:
                            self.current_worker.progress.emit(value)
                    
                    def status_callback(status):
                        if self.current_worker:
                            self.current_worker.status.emit(status)
            ...
            # 启动发布任务
            self.current_worker = self.thread_manager.start_task(publish_task)
            self.current_worker.progress.connect(self.progress_bar.setValue)
            self.current_worker.status.connect(self.status_label.setText)
            self.current_worker.finished.connect(self.on_publish_finished)
    ...
    def import_and_publish(self):
        ...
        try:
            ...
            # 启动任务
            self.current_worker = self.thread_manager.start_task(import_publish_task)
            self.current_worker.progress.connect(self.progress_bar.setValue)
            self.current_worker.status.connect(self.status_label.setText)
            self.current_worker.finished.connect(self.on_import_publish_finished)
...
def main():
    ...
    try:
        ...
        # 显示启动信息
        startup_message = (
            "欢迎使用 GeoServer & PostgreSQL 数据管理系统 V2.2\n\n"
            "主要改进:\n"
            "• 修复了线程管理问题，避免应用崩溃\n"
            "• 改进了任务取消机制\n"
            "• 增强了错误处理和异常恢复\n"
